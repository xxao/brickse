...
def request(url, parameters={}, post=False):
    ...
    # set default API key
    parameters['apiKey'] = assert_api_key(parameters.get('apiKey', None))
...
def assert_api_key(api_key):
    ...
    # check key
    if not api_key:
        raise ValueError("API key not specified. Run brickse.init() to set your key for all functions.")
...
def assert_user_token(user_token):
    ...
    # check token
    if not user_token:
        raise ValueError("User token must be specified. Run brickse.init() to set your token for all functions.")
